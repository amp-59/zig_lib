...
pub fn main() !void {
    ...
    var loader: Loader = .{};
    if (loader.load(mem.terminate("zig-out/lib/libzero-proc.so", 0))) |ptr| {
        const fp = @as(*fn (*Builder.FunctionPointers) void, @ptrCast(ptr));
        fp(&fps);
    } else |err| {
        return err;
    }
    if (loader.load(mem.terminate("zig-out/lib/libzero-build.so", 0))) |ptr| {
        const fp = @as(*fn (*Builder.FunctionPointers) void, @ptrCast(ptr));
        fp(&fps);
    } else |err| {
        return err;
    }
    if (loader.load(mem.terminate("zig-out/lib/libzero-format.so", 0))) |ptr| {
        const fp = @as(*fn (*Builder.FunctionPointers) void, @ptrCast(ptr));
        fp(&fps);
    } else |err| {
        return err;
    }
    if (loader.load(mem.terminate("zig-out/lib/libzero-about.so", 0))) |ptr| {
        const fp = @as(*fn (*Builder.FunctionPointers) void, @ptrCast(ptr));
        fp(&fps);
    } else |err| {
        return err;
