...
pub inline fn fieldIdentifier(comptime field_name: [:0]const u8) []const u8 {
...
pub inline fn fieldTagName(comptime field_name: [:0]const u8) []const u8 {
    @setEvalBranchQuota(~@as(u32, 0));
    comptime {
        const field_init: []const u8 = fieldInitializer(field_name);
...
fn FieldInitializer(comptime field_name: [:0]const u8) type {
    ...
    return *const [type_name.len -% (30 +% @typeName(Union).len)]u8;
}
pub inline fn fieldInitializer(comptime field_name: [:0]const u8) []const u8 {
    @setEvalBranchQuota(~@as(u32, 0));
    comptime {
        var type_info = @typeInfo(union {});
        type_info.Union.fields = &.{.{ .type = void, .name = field_name, .alignment = 1 }};
...
pub fn AnyFormat(comptime spec: RenderSpec, comptime T: type) type {
    ...
    switch (@typeInfo(T)) {
        ...
        .Pointer => |pointer_info| switch (pointer_info.size) {
            ...
            .Many, .C => return PointerManyFormat(spec, T),
...
pub fn StructFormat(comptime spec: RenderSpec, comptime Struct: type) type {
    ...
    const T = struct {
        ...
        pub const write: if (builtin.requireComptime(Struct))
            *const fn (buf: [*]u8, comptime Struct) [*]u8
        else
            *const fn (buf: [*]u8, Struct) [*]u8 = @ptrCast(&writeAny);

        fn writeAny(buf: [*]u8, value: anytype) [*]u8 {
            @setRuntimeSafety(fmt_is_safe);
                    ...
                    if (field_type_info.Pointer.size == .Many) {
                        if (spec.views.extern_slice) {
                            ...
                            if (@hasField(Struct, len_field_name)) {
                                ...
                                ptr = strcpyEqu(ptr, comptime fieldInitializer(field.name));
        ...
        pub const length: if (builtin.requireComptime(Struct))
            *const fn (comptime Struct) usize
        else
            *const fn (Struct) usize = @ptrCast(&lengthAny);

        fn lengthAny(value: anytype) usize {
            @setRuntimeSafety(fmt_is_safe);
            var len: usize = type_name.len +% 2;
            var fields_len: usize = 0;
            comptime var field_idx: usize = 0;
        ...
        pub fn write(buf: [*]u8, value: anytype) [*]u8 {
            ...
            inline for (fields) |field| {
                if (value == @field(tag_type.?, field.name)) {
                    if (field.type == void) {
                        ptr += neg2;
                        ptr = strcpyEqu(ptr, comptime fieldIdentifier(field.name));
                    } else {
                        ptr = strcpyEqu(ptr, comptime fieldInitializer(field.name));
