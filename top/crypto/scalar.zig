...
pub fn rejectNonCanonical(s: CompressedScalar) errors.NonCanonicalError!void {
    var c: u8 = 0;
    var n: u8 = 1;
    var i: usize = 31;
    while (true) : (i -= 1) {
...
pub fn randomX() CompressedScalar {
    return Scalar.randomBelow().toBytes();
}
...
pub const Scalar = struct {
    ...
    pub fn fromBytes(bytes: CompressedScalar) Scalar {
        var scalar: ScalarDouble = ScalarDouble.fromBytes32(bytes);
        return scalar.reduce(5);
    }
    /// Unpack a 64-byte representation of a scalar
    pub fn fromBytes64(bytes: [64]u8) Scalar {
        var scalar: ScalarDouble = ScalarDouble.fromBytes64(bytes);
        return scalar.reduce(5);
    }
    ...
    pub fn toBytes(expanded: *const Scalar) CompressedScalar {
        ...
        inline for (0..4) |idx| {
            mem.writeIntLittle(u64, bytes[idx * 7 ..][0..8], expanded.limbs[idx]);
        }
        const rem: u32 = @intCast(expanded.limbs[4]);
        mem.writeIntLittle(u32, bytes[28..][0..4], rem);
        return bytes;
    }
    ...
    pub fn add(x: Scalar, y: Scalar) Scalar {
        @setRuntimeSafety(false);
        const carry0: u64 = (x.limbs[0] + y.limbs[0]) >> 56;
        const t0: u64 = (x.limbs[0] + y.limbs[0]) & 0xffffffffffffff;
        const carry1: u64 = (x.limbs[1] + y.limbs[1] + carry0) >> 56;
        const t1: u64 = (x.limbs[1] + y.limbs[1] + carry0) & 0xffffffffffffff;
        const carry2: u64 = (x.limbs[2] + y.limbs[2] + carry1) >> 56;
        const t2: u64 = (x.limbs[2] + y.limbs[2] + carry1) & 0xffffffffffffff;
        const carry: u64 = (x.limbs[3] + y.limbs[3] + carry2) >> 56;
        const t3: u64 = (x.limbs[3] + y.limbs[3] + carry2) & 0xffffffffffffff;
        const t4: u64 = x.limbs[4] + y.limbs[4] + carry;
        const y01: u64 = 5175514460705773;
        const y11: u64 = 70332060721272408;
        const y21: u64 = 5342;
        const y31: u64 = 0;
        const y41: u64 = 268435456;
        const b5: u64 = (t0 -% y01) >> 63;
        const b6: u64 = (t1 -% (y11 + b5)) >> 63;
        const b7: u64 = (t2 -% (y21 + b6)) >> 63;
        const b8: u64 = (t3 -% (y31 + b7)) >> 63;
        const b: u64 = (t4 -% (y41 + b8)) >> 63;
        const mask: u64 = (b -% 1);
        return Scalar{
            .limbs = .{
                t0 ^ (mask & (t0 ^ ((b5 << 56) + t0) -% y01)),
                t1 ^ (mask & (t1 ^ ((b6 << 56) + t1) -% (y11 + b5))),
                t2 ^ (mask & (t2 ^ ((b7 << 56) + t2) -% (y21 + b6))),
                t3 ^ (mask & (t3 ^ ((b8 << 56) + t3) -% (y31 + b7))),
                t4 ^ (mask & (t4 ^ ((b << 56) + t4) -% (y41 + b8))),
            },
        };
    }
    /// Return x*r (mod L)
    pub fn mul(x: Scalar, y: Scalar) Scalar {
        @setRuntimeSafety(false);
        const xl0: u128 = x.limbs[0];
        const xl1: u128 = x.limbs[1];
        const xl2: u128 = x.limbs[2];
        const xl3: u128 = x.limbs[3];
        const xl4: u128 = x.limbs[4];
        const yl0: u128 = y.limbs[0];
        const yl1: u128 = y.limbs[1];
        const yl2: u128 = y.limbs[2];
        const yl3: u128 = y.limbs[3];
        const yl4: u128 = y.limbs[4];
        const xy000: u128 = xl0 *% yl0;
        const xy010: u128 = xl0 *% yl1;
        const xy020: u128 = xl0 *% yl2;
        const xy030: u128 = xl0 *% yl3;
        const xy040: u128 = xl0 *% yl4;
        const xy100: u128 = xl1 *% yl0;
        const xy110: u128 = xl1 *% yl1;
        const xy120: u128 = xl1 *% yl2;
        const xy130: u128 = xl1 *% yl3;
        const xy140: u128 = xl1 *% yl4;
        const xy200: u128 = xl2 *% yl0;
        const xy210: u128 = xl2 *% yl1;
        const xy220: u128 = xl2 *% yl2;
        const xy230: u128 = xl2 *% yl3;
        const xy240: u128 = xl2 *% yl4;
        const xy300: u128 = xl3 *% yl0;
        const xy310: u128 = xl3 *% yl1;
        const xy320: u128 = xl3 *% yl2;
        const xy330: u128 = xl3 *% yl3;
        const xy340: u128 = xl3 *% yl4;
        const xy400: u128 = xl4 *% yl0;
        const xy410: u128 = xl4 *% yl1;
        const xy420: u128 = xl4 *% yl2;
        const xy430: u128 = xl4 *% yl3;
        const xy440: u128 = xl4 *% yl4;
        const z00 = xy000;
