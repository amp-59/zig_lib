...
pub const P256 = struct {
    ...
    pub const fiat = @import("p256/p256_64.zig");
    ...
    pub const AffineCoordinates = struct {
        ...
        pub const identity_element: AffineCoordinates = .{
            .x = P256.identity_element.x,
            .y = P256.identity_element.y,
        };
    ...
    pub const identity_element: P256 = .{
        .x = Fe.zero,
        .y = Fe.one,
        .z = Fe.zero,
    };
    ...
    pub fn rejectIdentity(p: P256) errors.IdentityElementError!void {
        if (p.x.isZero()) {
            return error.IdentityElement;
        }
    }
    /// Create a point from affine coordinates after checking that they match the curve equation.
    pub fn fromAffineCoordinates(p: AffineCoordinates) errors.EncodingError!P256 {
        const x = p.x;
        const y = p.y;
        ...
        ret.z.cMov(P256.identity_element.z, is_identity);
        return ret;
    }
    /// Create a point from serialized affine coordinates.
    pub fn fromSerializedAffineCoordinates(xs: [32]u8, ys: [32]u8, endian: builtin.Endian) (errors.NonCanonicalError || errors.EncodingError)!P256 {
    ...
    pub fn fromSec1(s: []const u8) (errors.EncodingError || errors.NotSquareError || errors.NonCanonicalError)!P256 {
    ...
    fn pcMul(pc: *const [9]P256, s: [32]u8, comptime vartime: bool) errors.IdentityElementError!P256 {
        debug.assert(vartime);
        const e = slide(s);
        var q = P256.identity_element;
        var pos = e.len - 1;
        while (true) : (pos -= 1) {
            const slot = e[pos];
            if (slot > 0) {
    ...
    fn pcMul16(pc: *const [16]P256, s: [32]u8, comptime vartime: bool) errors.IdentityElementError!P256 {
        var q = P256.identity_element;
        var pos: usize = 252;
        while (true) : (pos -= 4) {
    ...
    pub fn mulDoubleBasePublic(p1: P256, s1_: [32]u8, p2: P256, s2_: [32]u8, endian: builtin.Endian) errors.IdentityElementError!P256 {
        ...
        const pc2 = if (p2.is_base) tab.base_point_pc_p256[0..9] else pc: {
