...
pub fn rejectNonCanonical(s: CompressedScalar, endian: builtin.Endian) errors.NonCanonicalError!void {
...
/// Return a*b (mod L)
pub fn mul(a: CompressedScalar, b: CompressedScalar, endian: builtin.Endian) errors.NonCanonicalError!CompressedScalar {
    return (try Scalar.fromBytes(a, endian)).mul(try Scalar.fromBytes(b, endian)).toBytes(endian);
}
/// Return a*b+c (mod L)
pub fn mulAdd(a: CompressedScalar, b: CompressedScalar, c: CompressedScalar, endian: builtin.Endian) errors.NonCanonicalError!CompressedScalar {
    return (try Scalar.fromBytes(a, endian)).mul(try Scalar.fromBytes(b, endian)).add(try Scalar.fromBytes(c, endian)).toBytes(endian);
}
/// Return a+b (mod L)
pub fn add(a: CompressedScalar, b: CompressedScalar, endian: builtin.Endian) errors.NonCanonicalError!CompressedScalar {
    return (try Scalar.fromBytes(a, endian)).add(try Scalar.fromBytes(b, endian)).toBytes(endian);
}
/// Return -s (mod L)
pub fn neg(s: CompressedScalar, endian: builtin.Endian) errors.NonCanonicalError!CompressedScalar {
    return (try Scalar.fromBytes(s, endian)).neg().toBytes(endian);
}
/// Return (a-b) (mod L)
pub fn sub(a: CompressedScalar, b: CompressedScalar, endian: builtin.Endian) errors.NonCanonicalError!CompressedScalar {
    return (try Scalar.fromBytes(a, endian)).sub(try Scalar.fromBytes(b, endian)).toBytes(endian);
}
/// Return a random scalar
pub fn randomX(endian: builtin.Endian) CompressedScalar {
    return Scalar.randomBelow().toBytes(endian);
}
...
pub const Scalar = struct {
    ...
    pub fn fromBytes(s: CompressedScalar, endian: builtin.Endian) errors.NonCanonicalError!Scalar {
    ...
    pub fn sqrt(n: Scalar) errors.NotSquareError!Scalar {
...
const null_field: Fe = builtin.zero(Fe);
const ScalarDouble = struct {
    x1: Fe,
    x2: Fe,
    fn fromBytes(comptime bits: usize, s_: [bits / 8]u8, endian: builtin.Endian) ScalarDouble {
        builtin.assert(bits > 0 and bits <= 512 and bits >= Fe.saturated_bits and bits <= Fe.saturated_bits * 2);
        var s = s_;
        if (endian == .Big) {
    ...
    fn reduce(expanded: ScalarDouble, comptime bits: usize) Scalar {
        ...
        var fe = expanded.x1;
        if (bits >= 256) {
            const st1 = Fe.fromInt(1 << 256) catch null_field;
