...
pub const about = struct {
    ...
    pub fn aboutFdAddrLenOffsetNotice(about_s: fmt.AboutSrc, fd: usize, addr: usize, len: usize, offset: usize) void {
        @setRuntimeSafety(false);
    ...
    pub fn aboutFdAddrLenOffsetProtFlagsNotice(
    ...
    fn copyNotice(dest_fd: usize, dest_offset: ?*usize, src_fd: usize, src_offset: ?*usize, max_count: usize, act_count: usize) void {
        ...
        ptr[0..7].* = "src_fd=".*;
        ptr = fmt.Udsize.write(ptr + 7, src_fd);
        ptr[0..10].* = ", dest_fd=".*;
        ptr = fmt.Udsize.write(ptr + 10, dest_fd);
        ptr[0..2].* = ", ".*;
        ptr = fmt.Udsize.write(ptr + 2, act_count);
        ptr[0] = '/';
    ...
    fn seekNotice(fd: usize, offset: usize, whence: Whence, to: usize) void {
        ...
        ptr = fmt.Ud64.write(ptr + 3, fd);
        ptr[0..6].* = ", cur=".*;
        ptr = fmt.Ud64.write(ptr + 6, to);
        if (whence != .set) {
            ptr[0..2].* = ", ".*;
            ptr = fmt.strcpyEqu(ptr + 2, @tagName(whence));
            ptr[0] = '+';
            ptr = fmt.Ud64.write(ptr + 1, offset);
        }
        ptr[0] = '\n';
        debug.write(buf[0 .. @intFromPtr(ptr + 1) -% @intFromPtr(&buf)]);
    }
    fn pollNotice(pollfds: []PollFd, timeout: usize) void {
        ...
        ptr[0..3].* = "fd=".*;
        ptr = fmt.Ud64.write(ptr + 3, pollfds.len);
        ptr[0..10].* = ", timeout=".*;
        ptr = fmt.Ud64.write(ptr + 10, timeout);
        ptr[0..3].* = "ms\n".*;
    ...
    fn listenNotice(sock_fd: usize, backlog: usize) void {
        ...
        ptr[0..8].* = "sock_fd=".*;
        ptr = fmt.Ud64.write(ptr + 8, sock_fd);
        ptr[0..10].* = ", backlog=".*;
        ptr = fmt.Ud64.write(ptr + 10, backlog);
        ptr[0] = '\n';
        debug.write(buf[0 .. @intFromPtr(ptr + 1) -% @intFromPtr(&buf)]);
    }
    ...
    fn writeUpdateFdOffset(buf: [*]u8, fd: usize, offset: usize) [*]u8 {
        ...
        ptr[0..7].* = "offset=".*;
        ptr = fmt.Udsize.write(ptr + 7, offset);
        ptr[0] = '\n';
        return ptr + 1;
    }
