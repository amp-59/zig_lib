...
// zig fmt: off
pub const abstract_specs: []const types.AbstractSpecification = &[_]types.AbstractSpecification{
    ...
    //.{ .kind = .dynamic,    .fields = lb_ss,        .layout = .structured,      .modes = rw_str,        .v_specs = s_dyn_specs, .v_techs = dyn_techs_1 },
    ...
    //.{ .kind = .dynamic,    .fields = lb_ss,        .layout = .unstructured,    .modes = rw_str,        .v_specs = u_dyn_specs, .v_techs = dyn_techs_1 },
...
const capacity_opts: types.Technique = mutually_exclusive_mandatory(.capacity, &.{
