...
fn mapContainersToParameters() void {
    ...
    array.writeMany(
        \\pub const DescrSet = struct {
        \\    params: gen.TypeDescrFormat,
        \\    specs: []const gen.TypeDescrFormat,
        \\    vars: gen.TypeDescrFormat,
        \\};
    );
    array.writeMany("pub const type_descrs=&[_]DescrSet{");
    inline for (out.type_specs) |type_spec| {
        array.writeMany(".{.params=");
        array.writeFormat(fmt.render(fmt_spec, comptime gen.ProtoTypeDescrFormat.init(type_spec.params)));
        array.writeMany(",\n");
        array.writeMany(".specs=&.{\n");
        inline for (type_spec.specs) |spec| {
            array.writeFormat(fmt.render(fmt_spec, comptime gen.ProtoTypeDescrFormat.init(spec)));
            array.writeMany(",\n");
        }
        array.writeMany("},\n");
        array.writeMany(".vars=");
        array.writeFormat(fmt.render(fmt_spec, comptime gen.ProtoTypeDescrFormat.init(type_spec.vars)));
        array.writeMany("},\n");
    }
