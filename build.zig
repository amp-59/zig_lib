...
pub fn buildMain(allocator: *build.Allocator, builder: *build.Builder) !void {
    ...
    const pathsplit: *build.Target       = builder.addTarget(small_spec, allocator, "pathsplit", "test/pathsplit.zig");
    const declprint: *build.Target       = builder.addTarget(debug_spec, allocator, "declprint", "test/print_all_decls.zig");
    // Other test programs:
    ...
    mg_container_specs.dependOnRun(allocator,   mg_container_args);
    mg_container_impls.dependOnRun(allocator,   mg_container_args);
    ...
    // zig fmt: on
    for ([_]*build.Target{
        builtin_test, meta_test, mem_test,    algo_test,   file_test,
        list_test,    fmt_test,  render_test, thread_test, virtual_test,
        build_test,
    }) |_| {}
