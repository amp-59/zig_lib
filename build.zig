...
fn examples(allocator: *build.Allocator, node: *Node) void {
    ...
    const statz: *Node = try node.addBuild(allocator, build_cmd, "statz", "examples/statz.zig");
    const perf: *Node = try node.addBuild(allocator, build_cmd, "perf", "examples/perf_events.zig");
...
fn tests(allocator: *build.Allocator, node: *Node) void {
    ...
    const serial_test: *Node = try node.addBuild(allocator, builder_exe, "serial_test", "test/serial-test.zig");
